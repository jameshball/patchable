...
public class XtAudioEngine implements AudioEngine {
  ...
  private Callable<float[]> channelGenerator;
  ...
  private int render(XtStream stream, Structs.XtBuffer buffer, Object user) throws Exception {
    ...
    for (int f = 0; f < buffer.frames; f++) {
      float[] channels = channelGenerator.call();
  ...
  private void writeChannels(float[] vector, Object output, int frame) {
  ...
  @Override
  public void play(Callable<float[]> channelGenerator, AudioDevice device) {
    this.playing = true;
