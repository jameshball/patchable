...
public class ConglomerateAudioEngine implements AudioEngine {
  ...
  @Override
  public void play(Callable<float[]> channelGenerator, AudioDevice device) throws Exception {
    playing = true;
    ...
    if (xtDevices != null && xtDevices.contains(device)) {
  ...
  private static class BufferedChannelGenerator implements Callable<float[]>, Runnable {

    private static final int BLOCK_SIZE = 1024;
    private static final int BUFFER_SIZE = 2;

    private final ArrayBlockingQueue<float[][]> buffer;
    private final Callable<float[]> channelGenerator;

    private float[][] currentBlock = null;
    private int currentVector = 0;
    private boolean stopped = true;

    private BufferedChannelGenerator(Callable<float[]> channelGenerator) {
